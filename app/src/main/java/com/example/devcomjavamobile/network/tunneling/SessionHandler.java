...
import com.example.devcomjavamobile.network.devcom.DataTrafficSender;
...
import com.example.devcomjavamobile.network.tunneling.transport.ip.IPPacketFactory;
...
import com.example.devcomjavamobile.network.tunneling.transport.PacketHeaderException;
import com.example.devcomjavamobile.network.tunneling.transport.icmp.ICMPPacket;
import com.example.devcomjavamobile.network.tunneling.transport.icmp.ICMPPacketFactory;
import com.example.devcomjavamobile.network.tunneling.transport.ip.IPv6Header;
            ...
            for(Peer p : peers) {
                ...
                if (p.getFingerPrint().equals(fingerPrint)) {
                    ...
                    // Send with TCP as fallback
                        ...
                        // Start TCP Control Packet with encrypted data length as a dedicated header only used for P(ackets)
                        ByteBuffer bb = ByteBuffer.allocate(4);
                        bb.putInt(encryptedBuffer.length);
                        byte[] encryptedDataLength = bb.array();
                        System.arraycopy(encryptedDataLength, 0, controlPacket, 23, 4);


                        p2p.newControlPacket(controlPacket, 'P', community, Utility.createFingerPrint());
                        Log.d(TAG, "Copying encrypted buffer");
                        System.arraycopy(encryptedBuffer, 0, controlPacket, 23 + 4, encryptedBuffer.length);
                        Log.d(TAG, "Signing");
